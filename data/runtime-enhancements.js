(function () {
  ...
  const autonomyMode = () => Boolean(window.FIDUCIAIRE_AUTONOMY);
  ...
  function installPracticalReviewTrustGuard() {
    ...
    window.FiduApp.savePracticalReview = function guardedSavePracticalReview(code) {
      const module = DATA.modules[code];
      if (!autonomyMode() && module && module.practicalReview && document.getElementById("practicalDecision")?.value === "passed") {
        let score = 0;
        ...
        for (const item of module.practicalReview.scoreItems || []) {
          ...
          if (!Number.isFinite(value) || value < 0 || value > item.max) { completeScores = false; break; }
          score += value;
        }
        const criticalFailure = (module.practicalReview.criticalChecks || []).some(
          (item) => document.getElementById(`critical-${item.id}`)?.value === "yes"
        );
        if (completeScores && (score < module.practicalReview.threshold || criticalFailure)) invalidatePersistedPracticalReview(code);
      }
  ...
  function enhanceRenderedPage() {
    ...
    setText(
      ...
      autonomyMode()
        ? `${pluralModules(state.published)} sur ${state.total} · progression autonome stockée dans ce navigateur · aucune certification automatique`
    ...
    setText(
      ...
      autonomyMode()
        ? `${pluralModules(state.published)} sur ${state.total}. Parcours autonome avec autocontrôle guidé et escalade ciblée; une revue humaine n’est pas exigée à chaque module.`
    ...
    if (module) {
      ...
      if (packageActions && module.learnerPackage && !module.learnerPackage.zip) {
        ...
        setText(
          ...
          autonomyMode()
            ? "Les modèles apprenant sont publiés séparément. Aucun fichier n’est téléversé vers la plateforme: complétez vos propres livrables puis marquez-les terminés."
      ...
      if (module.practicalReview && !autonomyMode()) {
        setText(
  ...
  function scheduleEnhancement() {
    if (scheduled) return;
    scheduled = true;
    queueMicrotask(() => { scheduled = false; enhanceRenderedPage(); });
  }

  installPracticalReviewTrustGuard();
  const app = document.getElementById("app");
  if (app && window.MutationObserver) new MutationObserver(scheduleEnhancement).observe(app, { childList: true, subtree: true });
