...
2. `roadmap-data.js`: pilote public v2.4, TC01 v1.3, autres fiches cœur, cas final et compléments de sources.
...
`modules.json` et `tracks.json` conservent surtout l’état v1.3 de la bibliothèque sectorielle. `sources-registry.json` est synchronisé avec les références TC01 actives (CO, Olico, LPD, PFPDT, LTVA/AFC, PA, LBA, OBA, FINMA, Zefix et IDE), dont la dernière revue éditoriale date du 06.08.2026. Cette date ne remplace jamais une vérification dans la source officielle avant usage. Ces fichiers ne doivent pas être utilisés seuls pour calculer la progression v2.4.
